puts "Using Ruby '#{RUBY_VERSION}' built for '#{RUBY_PLATFORM}' released on '#{RUBY_RELEASE_DATE}'\n\nHello World."
puts 'Press ESCAPE to exit'

s1 = Sprite.new
def test
	bitmap = Bitmap.new("test_resources/arc-logo")
	return
end

GC.start
test()
s1.bitmap = Bitmap.new("test_resources/arc-logo")
p s1.bitmap
s1.x = 100
GC.start
s1.bitmap.fill_rect(200, 0, 50, 50, Color.new(255, 0, 0))

s2 = Sprite.new
s2.bitmap = Bitmap.new(64, 64)
c = Color.new(255, 255, 255)
s2.bitmap.set_pixel(0, 0, c)
s2.bitmap.set_pixel(1, 0, c)
s2.bitmap.set_pixel(0, 1, c)
s2.bitmap.set_pixel(1, 1, c)
s2.bitmap.fill_rect(5, 5, 10, 10, Color.new(0, 255, 0, 128))
s2.bitmap.fill_rect(Rect.new(0, 30, 30, 30), Color.new(255, 255, 0, 128))

s1.bitmap.blt(240, 290, s2.bitmap, Rect.new(0, 0, 64, 64))

s3 = Sprite.new
s3.bitmap = s2.bitmap.clone
s3.bitmap.fill_rect(0, 0, 50, 50, Color.new(0, 128, 255))

c = s2.bitmap.get_pixel(5, 31)
p c.class
p c
s2.y = 100
s3.y = 300
s1.z = 10
GC.start
s2.bitmap.fill_rect(0, 0, s2.bitmap.width, s2.bitmap.height, Color.new(255, 255, 255))
s2.bitmap.fill_rect(s2.bitmap.width / 2, s2.bitmap.width / 2, s2.bitmap.width / 2, s2.bitmap.height / 2, Color.new(0, 255, 0))
s2.x = 32
s2.ox = 32
s2.oy = 32
s2.src_rect.set(16, 16, 40, 40)
p s2.src_rect
w1 = Window.new
#p1 = Plane.new()
viewport = Viewport.new(16, 16, 64, 64)
s11 = Sprite.new(viewport)
s11.z = 1000
s11.bitmap = Bitmap.new(64, 64)
s11.bitmap.fill_rect(0, 0, 64, 64, Color.new(255, 0, 0))
s12 = Sprite.new(viewport)
s12.z = 2000
s12.bitmap = Bitmap.new(64, 64)
s12.bitmap.fill_rect(0, 0, 64, 64, Color.new(0, 128, 255))
s12.x = -32
s12.y = 32
viewport.z = -100

#BEGIN _dump _load testing
puts "=================== BEGIN _dump and _load testing ==================="
def load_obj(filename)
  f = File.open(filename, "rb")
  data = Marshal.load(f)
  f.close()
  return data
end
def dump_obj(filename, obj)
  f = File.open(filename, "wb")
  Marshal.dump(obj, f)
  f.close()
end
puts "-----------------color testing----------------"
puts "loading a color"
color = load_obj("test_resources/Color.marshal")
puts "load test color"
p color.class
p color
p color.red
puts "dumping a color"
dump_obj("test_resources/Color_test.marshal", color)
puts "loading it again"
color = load_obj("test_resources/Color_test.marshal")
puts "load test color"
p color.class
p color
p color.red
puts "-----------------tone testing----------------"
puts "loading a tone"
tone = load_obj("test_resources/Tone.marshal")
puts "load test tone"
p tone.class
p tone
p tone.red
puts "dumping a tone"
dump_obj("test_resources/Tone_test.marshal", tone)
puts "loading it again"
tone = load_obj("test_resources/Tone_test.marshal")
puts "load test tone"
p tone.class
p tone
p tone.red
puts "-----------------table testing----------------"
puts "loading a table"
table = load_obj("test_resources/Table.marshal")
puts "load test ttable"
p table.class
p "0, 0, 0", table[0, 0, 0]
p "1, 1, 1", table[1, 1, 1]
puts "dumping a table"
dump_obj("test_resources/Table_test.marshal", table)
puts "loading it again"
table = load_obj("test_resources/Table_test.marshal")
puts "load test table"
p table.class
p "0, 0, 0", table[0, 0, 0]
p "1, 1, 1", table[1, 1, 1]
puts "-----------------rect testing----------------"
puts "loading a rect"
rect = load_obj("test_resources/Rect.marshal")
puts "load test rect"
p rect.class
p rect
p rect.width
puts "dumping a rect"
dump_obj("test_resources/Rect_test.marshal", rect)
puts "loading it again"
rect = load_obj("test_resources/Rect_test.marshal")
puts "load test rect"
p rect.class
p rect
p rect.width
puts "=================== END _dump and _load testing ==================="
#END _dump _load testing

loop do
	Graphics.update
	Input.update
	Graphics.frame_reset if Input.press?(Input::C)
	s1.y = 100 + Graphics::frame_count % 40
	#s2.x = 100 + Graphics::frame_count % 40
	s3.x = Graphics::frame_count % 40
	s3.y = 150 + Graphics::frame_count % 40
	s2.angle += 1
	s2.visible = Graphics::frame_count % 20 > 10
	break if Input.press?(Input::B)
end

