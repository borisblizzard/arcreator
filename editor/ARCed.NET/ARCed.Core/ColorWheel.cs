...
namespace ARCed.Core
{
	public class ColorWheel : IDisposable
	{
        ...
        internal TextureBrush AlphaBrush
        {
            get
            {
                if (_alphaImage == null)
                {
                    ...
                    using (Graphics g = Graphics.FromImage(_alphaImage))
                    {
		...
		// attempting to generate the _srcTexture. The color wheel
		...
		public ColorWheel(Rectangle colorRectangle, Rectangle brightnessRectangle, Rectangle selectedColorRectangle)
		{
			...
			using (var path = new GraphicsPath())
			{
				...
				// brightness rect, with a little extra 
				// "breathing room". 
				...
				// Create the image that contains the circular gradient.
				CreateGradient();
			}
		}
		...
		private void UpdateDisplay()
		{
			...
			{
				// Draw the saved color wheel _srcTexture.
				g.DrawImage(colorImage, colorRectangle);
				// Draw the "selected color" rect.

                g.FillRectangle(AlphaBrush, selectedColorRectangle);
		...
		private void DrawLinearGradient(Color TopColor)
		{
			...
			// brightness rect as the area to fill.
			using (var lgb =
		...
		private void CreateGradient()
		{
				...
				new PathGradientBrush(GetPoints(radius, new Point(radius, radius))))
			{
				...
				// work once. Later code uses the image
