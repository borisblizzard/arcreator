    ...
    public class AnnotationCollection : /*ICollection,*/ IEnumerable
    {
        ...
        public void CopyTo(Array array, int index)
        {
            ...
            if (index < 0 || index > array.Length)
                throw new ArgumentOutOfRangeException("index", _resources.Exception_IndexOutOfRange);

            if (array.Length - index < Count)
                throw new ArgumentException(_resources.Exception_InsufficientSpace);

            for (int lvl = 0; lvl < Count; lvl++)
                array.SetValue(this[lvl], index + lvl);
        }
        */
