    class WxRMXPMapWindow(wx.ScrolledWindow):
        ...
        def __init__(self, parent, id=-1, size=wx.DefaultSize, map_id=0):
            ...
            wx.ScrolledWindow.__init__(
                self, parent, id, (0, 0), size=size, style=wx.SUNKEN_BORDER)
            ...
            self.autotiles_b = []
            # layer buffers
            self.layer1 = wx.EmptyBitmapRGBA(
                32 * self.map.width, 32 * self.map.height)  # layer1
            self.layer2 = wx.EmptyBitmapRGBA(
                32 * self.map.width, 32 * self.map.height)  # layer2
            self.layer3 = wx.EmptyBitmapRGBA(
                32 * self.map.width, 32 * self.map.height)  # layer3
            self.layer4 = wx.EmptyBitmapRGBA(
                32 * self.map.width, 32 * self.map.height)  # layerEvents
            self.layer5 = wx.EmptyBitmapRGBA(
                32 * self.map.width, 32 * self.map.height)  # layerBrush
            self.layer6 = wx.EmptyBitmapRGBA(
                32 * self.map.width, 32 * self.map.height)  # layerMouse
            self.layerP = wx.EmptyBitmapRGBA(
                32 * self.map.width, 32 * self.map.height)  # layerPreview
        ...
        def DoDrawing(self, dc, init=False):
            ...
            if self.mode == self.LayerP:
                ...
                dc.DrawRectangle(
                    0, 0, size.width * self.zoom, size.height * self.zoom)
                ...
                dc.DrawBitmap(
                    layerP, 0, 0, size.width * self.zoom, size.height)
                ...
                if (layer1flag != True and layer2flag != True and layer3flag !=
                    ...
                    return
                dc.DrawRectangle(
                    0, 0, self.layer1.GetWidth() * self.zoom, self.layer1.GetWidth() * self.zoom)
                # layer 1
                ...
                imagelayer2.AdjustChannels(
                    1.0, 1.0, 1.0, self.LayerTransparencyFactor)
                ...
                # event layer
                dc.DrawBitmap(
                    self.layer4, 0, 0, self.layer4.GetWidth(), self.layer4.GetHeight())
                path = dc.CreatePath()
        ...
        def load_autotiles(self):
            for i in range(7):
                ...
                self.autotiles_b[i] = self.Cache.Autotile(
                    autotile_name, self.Project.Location)
                if not self.autotiles_b[i]:
                    self.autotiles_b[i] = self.Cache.Autotile(
                        autotile_name, self.Project.RTP_Location)
                if not self.autotiles_b[i]:
        ...
        def draw_mouse(self):
            ...
            dc.SetBackground(
                wx.Brush(wx.Colour(255, 255, 255, 0), wx.TRANSPARENT))
            dc.Clear()
            gc = wx.GraphicsContext.Create(dc)
            path = gc.CreatePath()
            path.AddRectangle(0, 0, 31, 31)
        ...
        def draw_preview(self):
            ...
            if self.mode == self.LayerP:
                ...
                dc.SetBackground(
                    wx.Brush(wx.Colour(255, 255, 255, 0), wx.TRANSPARENT))
                dc.Clear()
                tileset = self.Cache.Tileset(
                    self.tileset_name, self.Project.Location)
                if not tileset:
                    tileset = self.Cache.Tileset(
                        self.tileset_name, self.Project.RTP_Location)
                if not tileset:
                                    ...
                                    if p == self.tileset.Priorities[id]:
                                        ...
                                        if id < 384:
                                            if self.autotiles[id / 48 - 1].GetWidth() / 96 > 1:
                                                autotile = self.autotiles_b[
                                                    id / 48 - 1]
                                                tile_id = id % 48
                                                bitmap = wx.EmptyBitmapRGBA(
                                                    32, 32)
                                                # Collects Auto-Tile Tile
                                                # Layout
                                                tiles = self.Autotiles[
                                                    tile_id / 8][tile_id % 8]
                                                dc_at = wx.MemoryDC()
                                                ...
                                                for i in range(4):
                                                    tile_position = tiles[
                                                        i] - 1
                                                    src_rect = wx.Rect(
                                                        tile_position % 6 * 16, tile_position / 6 * 16, 16, 16)
                                                    sub_at_bitmap = autotile.GetSubBitmap(
                                                        src_rect)
                                                    dc_at.DrawBitmap(
                                                        sub_at_bitmap, i % 2 * 16, i / 2 * 16, True)
                                                dc_at.SelectObject(
                                                    wx.NullBitmap)
                                                t_x = x * 32
                                                t_y = y * 32
                                            ...
                                            rect = wx.Rect(
                                                (id - 384) % 8 * 32, (id - 384) / 8 * 32, 32, 32)
                                            sub_bitmap = tileset.GetSubBitmap(
                                                rect)
                                            dc.DrawBitmap(
                                                sub_bitmap, x * 32, y * 32, True)
                dc.SelectObject(wx.NullBitmap)
        ...
        def draw_events(self):
            ...
            for key in self.events.keys():
                ...
                gc.PopState()              # restore saved state
                gc.DrawBitmap(
                    bitmap, event.x * 32 + 5, event.y * 32 + 5, bitmapSize.width, bitmapSize.height)
            dc.SelectObject(wx.NullBitmap)
