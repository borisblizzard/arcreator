...
class ActionManager(object):
    ...
    def Undo():
        if ActionManager._current_action > 0:
            ActionManager.BatchAction(
                ActionManager._current_action, ActionManager._current_action - 1, 0)

    @staticmethod
    def Redo():
        if len(ActionManager._action_stack) > 1:
            ActionManager.BatchAction(
                ActionManager._current_action, ActionManager._current_action + 1, 1)
    ...
    def BatchAction(start, end, direction):
        ...
        if ActionManager._current_action >= len(ActionManager._action_stack):
            ActionManager._current_action = len(
                ActionManager._action_stack) - 1
        if not success:
            Kernel.Log(
