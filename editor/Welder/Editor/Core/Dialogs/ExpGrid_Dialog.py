...
class ExpGrid_Dialog(ExpGrid_Dialog_Template):
    ...
    def RefreshTable(self):
        ...
        for i in range(self.Actor.final_level):
            ...
            if self.PageIndex == 0:
                value = str(expList[i + 1] - expList[i])
            else:
                value = str(expList[i + 1])
            text = ''.join([value, "\t"])
    ...
    def GenerateCurvedList(self):
        ...
        for i in range(2, final + 1):
            values[i] = int(
                DM.CalculateParameter(min, max, speed, i, 2, final))
        return values
    ...
    def buttonGraphEditor_Clicked(self, event):
        ...
        maxV = max(expList)
        dlg = ExpGraphEditor_Dialog(
            self, data, 'Experience', 'blue', maxV, final, init)
        dlg.ShowModal()
