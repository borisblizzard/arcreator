...
class ChooseTreasure_Dialog(ChooseTreasure_Dialog_Template):
    ...
    def __init__(self, parent, current):
        ...
        self.listCtrlTreasure.InsertColumn(
            0, "Type", wx.LIST_FORMAT_CENTRE, 64)
        self.listCtrlTreasure.InsertColumn(
            1, "Chance", wx.LIST_FORMAT_CENTRE, 64)
        self.listCtrlTreasure.InsertColumn(2, "Treasure", width=160)
        self.listCtrlTreasure.InsertColumn(
            3, "Quantity", wx.LIST_FORMAT_CENTRE, 64)
        self._combos = [
    ...
    def refreshTreasureList(self):
        ...
        for i in range(3):
            if i == 0:
                type, src = 'Item', DataItems
            elif i == 1:
                type, src = 'Weapon', DataWeapons
            else:
                type, src = 'Armor', DataArmors
            for treasure in self.Treasure[i]:
                self.listCtrlTreasure.InsertStringItem(count, type)
                self.listCtrlTreasure.SetStringItem(
                    count, 1, str(treasure[1]) + '%')
                self.listCtrlTreasure.SetStringItem(
                    count, 2, src[treasure[0]].name)
    ...
    def listCtrlTreasure_KeyPressed(self, event):
        ...
        if event.GetKeyCode() == 0x007F:
            index = -1
            index = self.listCtrlTreasure.GetNextItem(
    ...
    def getTreasure(self, index, delete=False):
        ...
        if index < len(self.Treasure[0]):
            ...
            if delete:
                del(self.Treasure[2][
                    index - len(self.Treasure[0]) - len(self.Treasure[1])])
            else:
