import wx

import Kernel

from PyitectConsumes import AnimationEntireSlide_Dialog_Template
# -------------------------------------------------------------------------
# AnimationEntireSlide_Dialog
# -------------------------------------------------------------------------

# Implementing AnimationEntireSlide_Dialog
class AnimationEntireSlide_Dialog(AnimationEntireSlide_Dialog_Template):

    def __init__(self, parent):
        AnimationEntireSlide_Dialog_Template.__init__(self, parent)

    # Handlers for AnimationEntireSlide_Dialog events.
    def spinCtrlFramesStart_ValueChanged(self, event):
        # TODO: Implement spinCtrlFramesStart_ValueChanged
        pass

    def spinCtrlFramesEnd_ValueChanged(self, event):
        # TODO: Implement spinCtrlFramesEnd_ValueChanged
        pass

    def buttonOK_Click(self, event):
        # TODO: Implement buttonOK_Click
        pass

    def buttonCancel_Click(self, event):
        # TODO: Implement buttonCancel_Click
        pass
