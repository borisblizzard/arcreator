...
def load(io, redirects={}, extended_namespace={}):
    ...
    __class_path_redirects = redirects
    __io = io
    build_object_table(extended_namespace)
    link_objects()
    call_arc_post_load()
    object = __symbols[0]
...
def deep_tuple_to_list(t):
    return list(map(deep_tuple_to_list, t)) if isinstance(t, (list, tuple)) else t
  
def build_object_table(extended_namespace={}):
    global __io
    global __symbols
    ...
    for key, itemt in objects.items():
        __symbols[key] = deep_tuple_to_list(itemt)
        item = __symbols[key]
        if item[0] >= 0 and item[0] <= 4: #False, True, Nil, String, Numeric
            ...
            if extended_namespace.has_key(klasses[0]):
                klass = extended_namespace[klasses[0]]
            else:
                raise TypeError, "No class defined: %s in the provided namespace" % klasses[0]
            if len(klasses) > 1:
