...
class MainStatusBar(wx.StatusBar):
    def __init__(self, parent):
        ...
        #set default text
        self.SetStatusText("ARCed - Advanced RPG Creator Editor", 3)
...
class MapTreeCtrl(wx.TreeCtrl):
    ...
    def buildStruct(self):
        ...
        for key, value in mapinfos.iteritems():
            self.struct[key] = []
            if self.struct.has_key(value.parent_id):
    ...
    def Refresh_Map_List(self):
        ...
        for key in self.struct:
            ...
            for id in self.struct[key]:
                if self.maps.has_key(mapinfos[id].parent_id):
