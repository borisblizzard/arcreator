...
namespace hltypes
{
	...
	public:
		...
		bool nequals(const List<T>& other) const
		{
			if (this->size() != other.size())
			{
				return true;
		...
		/// @param[in] element Element to search for.
		/// @return Index of the given element or -1 if element could not be found.
		List<int> indexes_of(T element) const
		{
			List<int> result;
			const_iterator_t it = stdlist::begin();
			for_iter (i, 0, this->size())
		...
		List<T> remove_at(const int index, const int count)
		{
		...
		void remove(const List<T>& other)
		{
			int index;
			for_iter (i, 0, other.size())
			{
		...
		/// @param[in] other List of elements to remove.
		/// @return Number of elements removed.
		int remove_all(const List<T>& other)
		{
			List<int> indexes;
			iterator_t it;
			int count = 0;
		...
		void push_back(const List<T>& other, const int count)
		{
		...
		/// @param[in] count Number of elements to add.
		void push_back(const List<T>& other, const int start, const int count)
		{
		...
		/// @param[in] other List of elements to add.
		/// @param[in] count Number of elements to add.
		void push_front(const List<T>& other, const int count)
		{
		...
		/// @param[in] start Start index of the elements to add.
		/// @param[in] count Number of elements to add.
		void push_front(const List<T>& other, const int start, const int count)
		{
		...
		/// @return List of all removed elements.
		/// @note Elements in the returned List are in the same order as in the orignal List.
		List<T> pop_front(const int count)
		{
			if (count > this->size())
			{
				throw container_range_error(0, count);
		...
		/// @return List of all removed elements.
		/// @note Elements in the returned List are in the same order as in the orignal List.
		List<T> pop_back(const int count)
		{
			if (count > this->size())
			{
				throw container_range_error(0, count);
		...
		/// @param[in] element Element to unite with.
		/// @return A new List.
		List<T> united(const T& element) const
		{
			List<T> result(*this);
		...
		/// @param[in] other List to unite with.
		/// @return A new List.
		List<T> united(const List<T>& other) const
		{
			List<T> result(*this);
			result.unite(other);
			return result;
		...
		void intersect(const List<T>& other)
		{
			List<T> result;
			for_iter (i, 0, this->size())
			{
		...
		/// @param[in] other List to intersect with.
		/// @return A new List.
		List<T> intersected(const List<T>& other) const
		{
			List<T> result(*this);
		...
		void differentiate(const List<T>& other)
		{
			int index;
			for_iter (i, 0, other.size())
			{
		...
		/// @return A new List.
		/// @note Unlike remove, this method ignores if the element is not in this List.
		List<T> differentiated(const T& element) const
		{
			List<T> result(*this);
		...
		/// @return A new List.
		/// @note Unlike remove, this method ignore elements of other List that are not in this one.
		List<T> differentiated(const List<T>& other) const
		{
			List<T> result(*this);
		...
		/// @return A new List.
		List<T> removed_duplicates() const
		{
			List<T> result(*this);
		...
		/// @note The sorting order is ascending.
		/// @note compare_function should return true if first element is less than the second element.
		List<T> sorted(bool (*compare_function)(T, T)) const
		{
			List<T> result(*this);
		...
		List<T> random(int count, bool unique = false) const
		{
			List<T> result;
			if (!unique)
			{
		...
		/// @param[in] unique Whether to force all random values to be unique.
		/// @return List of random elements selected from this one.
		List<T> pop_random(int count, bool unique = false)
		{
			List<T> result;
			if (!unique)
			{
		...
		List<T> find_all(bool (*condition_function)(T))
		{
			List<T> result;
			for_iter (i, 0, this->size())
			{
		...
		template <class S>
		List<S> dyn_cast(bool include_nulls = false)
		{
			List<S> result;
			S value;
			for_iter (i, 0, this->size())
		...
		bool includes(const List<T>& other) const
		{
		...
		/// @brief Same as contains.
		/// @see contains(const List<T>& other)
		bool has(const List<T>& other) const
		{
			return this->contains(other);
		}
		/// @brief Same as contains.
		...
		/// @brief Same as contains.
		/// @see contains(const List<T>& other)
		bool has_element(const List<T>& other) const
		{
			return this->contains(other);
		}
		/// @brief Same as contains.
		...
		void add(const List<T>& other, const int count)
		{
			this->push_back(other, count);
		}
		/// @brief Same as push_back.
		/// @see push_back(const List<T>& other, const int start, const int count)
		void add(const List<T>& other, const int start, const int count)
		{
		...
		/// @brief Same as push_back.
		/// @see push_back(const List<T>& other, const int count)
		void append(const List<T>& other, const int count)
		{
			this->push_back(other, count);
		}
		/// @brief Same as push_back.
		/// @see push_back(const List<T>& other, const int start, const int count)
		void append(const List<T>& other, const int start, const int count)
		{
			this->push_back(other, start, count);
		}
		/// @brief Same as push_back.
		...
		/// @brief Same as push_back.
		/// @see push_back(const List<T>& other).
		void push_last(const List<T>& other)
		{
			this->push_back(other);
		}
		/// @brief Same as push_back.
		/// @see push_back(const List<T>& other, const int count).
		void push_last(const List<T>& other, const int count)
		{
			this->push_back(other, count);
		}
		/// @brief Same as push_back.
		/// @see push_back(const List<T>& other, const int start, const int count).
		void push_last(const List<T>& other, const int start, const int count)
		{
			this->push_back(other, start, count);
		}
		/// @brief Same as push_back.
		...
		List<T> pop_first(const int count)
		{
		...
		List<T> pop_last(const int count)
		{
		...
		/// @brief Same as pop_front.
		/// @see pop_front(const int count).
		List<T> remove_front(const int count)
		{
			return this->pop_front(count);
		}
		/// @brief Same as pop_back.
		...
		/// @brief Same as pop_back.
		/// @see pop_back(const int count).
		List<T> remove_back(const int count)
		{
			return this->pop_back(count);
		}
		/// @brief Same as pop_front.
		...
		/// @brief Same as pop_front.
		/// @see pop_front(const int count).
		List<T> remove_first(const int count)
		{
			return this->pop_front(count);
		}
		/// @brief Same as pop_back.
		...
		/// @brief Same as pop_back.
		/// @see pop_back(const int count).
		List<T> remove_last(const int count)
		{
			return this->pop_back(count);
		}
		...
		List<T> pop(const int index, const int count)
		{
		...
		/// @brief Same as remove_at.
		/// @see remove_at(const int index, const int count)
		List<T> pop_at(const int index, const int count)
		{
			return this->remove_at(index, count);
		}
		...
		int pop_all(const List<T>& other)
		{
	...
	private:
		...
		const_iterator_t _const_iterator_plus(const_iterator_t it, int count) const
		{
			for_iter (i, 0, count)
			{
				it++;
		...
		iterator_t _iterator_plus(iterator_t it, int count)
		{
			for_iter (i, 0, count)
			{
				it++;
