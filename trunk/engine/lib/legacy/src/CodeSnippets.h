#ifndef LEGACY_CODE_SNIPPETS_H
#define LEGACY_CODE_SNIPPETS_H
...
#define RB_CANT_CLONE_ERROR(value) \
	rb_raise(rb_eTypeError, ("Can't clone: " + value->typeName).cStr());
/// @brief Throws a duping error.
/// @param[in] value Value that can't be duplicated.
#define RB_CANT_DUP_ERROR(value) \
	rb_raise(rb_eTypeError, ("Can't dup: " + value->typeName).cStr());
...
#define RB_CHECK_DISPOSED(value) \
	if (value->disposed) \
	{ \
		rb_raise(rb_eRGSSError, ("Disposed: " + value->typeName).cStr()); \
	}
...
#define RB_CHECK_TYPE(var, type1) \
	...
	{ \
		...
		rb_raise(rb_eTypeError, message.cStr()); \
	}
